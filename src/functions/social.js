...
export const SOCIAL_FUNCTIONS = [
    {
        ...
        officialDocsNote:
            'The community guide flags this as usable only inside Microsites and Landing Pages and only for content regions built in Classic Content, which is retired. A reached call aborts the page with HTTP 422 and no partial output for both a string region name and a numeric region id; gating the same call behind an unmatched query-string branch leaves the page at HTTP 200, confirming a runtime abort of the reached call rather than a compile-time failure. No Classic Content region exists to source a resolvable identifier, so the documented success path cannot be exercised. Email/send context: the send parser EXPLICITLY rejects this function with errorcode 10004 ("GetPublishedSocialContent Function is only allowed to be called from social sharing pages or landing pages") - a hard context restriction, stronger and different from the CloudPage runtime abort, which only occurs because no Classic Content region resolves.',
    },
    {
